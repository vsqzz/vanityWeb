...
export async function RedeemKey(serial: string, user_id: string) {
    ...
    await sql`UPDATE Vanityﾒ𝟶_keys SET claimed = true, claimed_discord_id = ${user_id}, lrm_serial = ${keyCreation.user_key} WHERE serial = ${serial}`;
    ...
    if ((rows[0].order_id as string).toLowerCase().includes("bloxproducts")) {
        await fetch(`${process.env.BLOXPRODUCTS_WEBHOOK}`, {
            ...
            body: JSON.stringify({
                ...
                "embeds": [
                  {
                    "title": "BLOXPRODUCTS - Vanityﾒ𝟶 key purchased",
                    "description": "a key was purchased & redeemed via the bloxproducts.",
                    "color": 5814783,
                    "fields": [
                      {
                        "name": "Vanityﾒ𝟶 serial",
                        "value": `||${serial}||`,
                        "inline": true
                      },
                      {
...
export async function GenerateSerial(invoiceID: string | null, amount: number) {
    ...
    for (let i = 0; i < amount; i++) {
        ...
        await sql`INSERT INTO Vanityﾒ𝟶_keys (serial, order_id, claimed) VALUES (${serial}, ${actualInvoiceId}, false);`;
        serials.push(serial);
    }
...
export async function DeleteSerial(serial: string) {
    ...
    await sql`DELETE FROM Vanityﾒ𝟶_keys WHERE serial = ${serial}`;
    return {
        status: 200,
...
export async function GetAllSerialData() {
    ...
    const { rows } = await sql`SELECT * FROM Vanityﾒ𝟶_keys`;
    return rows;
}