...
async function getIp(headersList: Headers, request: NextRequest) {
    ...
    const Vanityﾒ𝟶IP = headersList.get("x-Vanityﾒ𝟶-ip");
    if (Vanityﾒ𝟶IP && Vanityﾒ𝟶IP !== "") return Vanityﾒ𝟶IP;
...
export async function POST(request: NextRequest) {
    ...
    if (hash === signature) {
        ...
        for (let i = 0; i < data.quantity; i++) {
            ...
            await sql`INSERT INTO Vanityﾒ𝟶_keys (serial, order_id, claimed) VALUES (${serial}, ${data.invoice.id}, false);`;
            createdSerials.push(serial);
        }
        
        return new Response(`Thank you for purchasing ${data.quantity} Vanityﾒ𝟶 key(s)!\nYou can redeem your serial(s) at https://www.Vanityﾒ𝟶.cc/purchase/completed?serial=${encodeURIComponent(createdSerials.join(","))}\n\nMake sure to keep this link safe, as it is the only way to redeem your key(s).`);
    } else {
...
export async function GET(request: NextRequest) {
    ...
    await sql`CREATE TABLE IF NOT EXISTS Vanityﾒ𝟶_keys ( serial TEXT PRIMARY KEY, order_id TEXT NOT NULL, claimed BOOL, claimed_discord_id TEXT, lrm_serial TEXT );`
    const { rows } = await sql`SELECT * FROM Vanityﾒ𝟶_keys WHERE order_id = ${order_id};`
